...
class combined_thresh(object):
    ...
    def run(self, image):
        self.image = image
        self.poll()
        return self.threshholded.tolist()
    ...
    def poll(self):
        ...
        l_channel = luv[:, :, 0]
        binary_output_luv = np.zeros_like(l_channel)
        binary_output_luv[(l_channel > luv_thresh[0]) & (l_channel <= luv_thresh[1])] = 1
        ...
        #self.threshholded[((x_threshd == 1) & (mag_threshd == 1)) | ((dir_threshd == 1) & (hls_threshd == 1)) | (luv_threshd == 1)] = 255
        #self.threshholded[(mag_threshd == 1)] = 255
        self.threshholded[(((x_threshd == 1) & (mag_threshd == 1)) | (self.maskw == 255)) | (luv_threshd == 1) ] = 255
        #self.threshholded = cv2.rotate(self.threshholded, cv2.ROTATE_90_CLOCKWISE)



if __name__ == '__main__':

    img = cv2.imread('/home/pi/testbed/mycar/custom_parts/img/testimg2.jpg')
    C = combined_thresh()
    threshed = C.run(img)
    writeStatus = cv2.imwrite('/home/pi/testbed/mycar/custom_parts/img/thresh_testimg2.jpg', threshed) 
    if writeStatus is True:
        print("Thresh image successfully written")
    else:
        print("problem")